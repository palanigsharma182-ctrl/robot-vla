...
usage() {
    echo "用法: $0 <VLA/datasets 下的远端子目录> <本地目标目录>" >&2
}
...
if [[ -z "$remote_name" || "$remote_name" == /* || "/$remote_name/" == *"/../"* ]]; then
    echo "错误: 远端子目录必须是 VLA/datasets 下的相对路径" >&2
    exit 2
fi
...
source_path="${rclone_remote}:VLA/datasets/$remote_name"
echo "下载数据集: $source_path -> $destination"
