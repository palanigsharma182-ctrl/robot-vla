...
def quaternion_angular_distance_rad(
    ...
    one = _finite_vector(first, 4, "first quaternion")
    two = _finite_vector(second, 4, "second quaternion")
    one_norm = float(np.linalg.norm(one))
    two_norm = float(np.linalg.norm(two))
    if one_norm <= 1e-12 or two_norm <= 1e-12:
        raise ValueError("四元数范数必须为正")
    cosine_half_angle = float(abs(np.dot(one / one_norm, two / two_norm)))
...
def compose_camera_orientation_wxyz(
    ...
    nominal = _finite_vector(nominal_quaternion_wxyz, 4, "nominal quaternion")
    nominal_norm = float(np.linalg.norm(nominal))
    if nominal_norm <= 1e-12:
        raise ValueError("nominal quaternion 范数必须为正")
    nominal = nominal / nominal_norm
