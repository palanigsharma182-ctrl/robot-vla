...
```text
云端 RTX 4090 24GB（SSH alias: robot-vla-4090）
```

涉及 CUDA / BF16、正式 checkpoint 数值复现、模型训练或中大型推理实验时，优先在云端
4090 上执行。本地环境只用于代码搜索、静态检查、纯函数单元测试和小规模 CPU smoke；
不要为了正式 GPU 实验先在本地安装大体积 CUDA / PyTorch 依赖。

云端不可用、正式数据只在本地或任务明确要求本地运行时，再回退到本地；回退前说明原因。
