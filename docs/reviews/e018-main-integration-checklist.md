...
## 最新接入记录：2026-09-06 真实 V1 基线及暂停恢复

正式 G2C/D049 消费和固定阶段的真实 V1 VLA 暂停→重观察→HOME→fresh replan→继续执行已完成工程验收。
真实 VLA 为冻结 V1/Layer-12 历史基线，不是 V2 训练权重。完整证据、独立审查和未验证范围见
[33 项对应表的最新追加验收](capability-integration-matrix.md)。以下静态清单与早期“待接入”描述
保持原审查时点，不应覆盖上述最新工程状态；Stage2A历史负结果和Stage2B/3状态未变。

## 前一阶段接入记录：2026-09-06 33 项对应关系与模型接线
