...
def test_delayed_commit_preserves_rgb_acquisition_time_with_sensor_skew() -> None:
    orchestrator = _orchestrator()
    for index in range(3):
        frame = _frame(index, timestamp_s=0.05 + index * 0.05)
        frame = replace(frame, rgb_timestamp_s=frame.control_timestamp_s - 0.005)
        orchestrator.observe_collect_frame(frame, safety=_safety())
    _pass_home_barrier(orchestrator)
    assert _recheck(orchestrator)
    _commit(orchestrator)
    assert orchestrator.memory.state.last_observed_timestamp_s == pytest.approx(0.145)
    assert orchestrator.memory.state.state_timestamp_s == pytest.approx(0.4)
    assert orchestrator.memory_write_count == 1


def test_rgb_age_expiry_rejects_without_applying_preview_age() -> None:
    orchestrator = _orchestrator()
    for index in range(3):
        frame = _frame(index, timestamp_s=0.05 + index * 0.05)
        frame = replace(frame, rgb_timestamp_s=frame.control_timestamp_s - 0.005)
        orchestrator.observe_collect_frame(frame, safety=_safety())
    _pass_home_barrier(orchestrator)
    assert _recheck(orchestrator, timestamp_s=2.647)
    before = orchestrator.memory.state
    with pytest.raises(RuntimeError, match="delayed Object Memory commit 被拒绝"):
        _commit(orchestrator, timestamp_s=2.647)
    assert orchestrator.no_commit_receipt is not None
    assert not orchestrator.no_commit_receipt.prior_memory_safety_invalidated
    assert "memory_stale_at_commit" in orchestrator.no_commit_receipt.rejection_reasons
    assert orchestrator.no_commit_receipt.pre_state_digest == orchestrator.no_commit_receipt.post_state_digest
    assert orchestrator.state is PendingActiveViewState.HOME_VERIFIED_FAILED_SAFE_HOLD
    assert orchestrator.shadow_action_receipt is None
    assert orchestrator.memory.state == before
    assert orchestrator.memory_write_count == 0
