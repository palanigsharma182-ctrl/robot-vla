...
def test_calibration_identity_rejects_inconsistent_scale_and_order_statistic() -> None:
    for override in ({"scale_factor": 1.0}, {"order_statistic_k": 38}, {"support_count": 50}):
        with pytest.raises(ValueError, match="k/scale"):
            replace(_calibration(), **override)


def test_covariance_roundoff_is_not_amplified_into_negative_variance() -> None:
    for scale in (4.0, 1e6):
        covariance = np.diag([1e-6, 1e-6, -5e-13])
        covariance[0, 1] = 5e-13
        result = _calibration(scale).calibrate_covariance(covariance)
